 ...
 * @author laim0nas100
 */
public abstract class BasedJpaQueryDecor<T_ROOT, T_RESULT, CTX, M extends BasedJpaQueryDecor<T_ROOT, T_RESULT, CTX, M>> {
    ...
    protected BasedJpaQueryDecor(CTX context, Class<T_ROOT> rootClass, Class<T_RESULT> resultClass, BasedJpaQueryDecor copy) {
        this(copy);
    ...
    protected BasedJpaQueryDecor(BasedJpaQueryDecor copy) {
        if (copy != null) {
    ...
    public abstract <NEW_ROOT extends T_ROOT> BasedJpaQueryDecor<NEW_ROOT, T_RESULT, CTX, ?> usingSubtype(Class<NEW_ROOT> subtype);

    public BasedJpaQueryDecor<T_ROOT, T_ROOT, CTX, ?> selectingRoot() {
        return selecting(rootClass, p -> p.root());
    }

    public abstract <RES> BasedJpaQueryDecor<T_ROOT, RES, CTX, ?> selecting(
            Class<RES> resClass, Function<Phase2<T_ROOT, CTX>, Expression<RES>> func);

    public <RES> BasedJpaQueryDecor<T_ROOT, RES, CTX, ?> selecting(
    ...
    public abstract BasedJpaQueryDecor<T_ROOT, Tuple, CTX, ?> selectingTuple(Function<Phase2<T_ROOT, CTX>, List<Selection<?>>> selections);

    public BasedJpaQueryDecor<T_ROOT, Tuple, CTX, ?> selectingTuple(List<Selection<?>> selections) {
        return selectingTuple(p -> selections);
    }

    public BasedJpaQueryDecor<T_ROOT, Tuple, CTX, ?> selectingTuple(Selection<?>... selections) {
        return selectingTuple(Arrays.asList(selections));
    }

    public BasedJpaQueryDecor<T_ROOT, Tuple, CTX, ?> selectingTuple(SingularAttribute<? super T_ROOT, ?>... selections) {
    ...
    public M withPred(Function<Phase2<T_ROOT, CTX>, Predicate> func) {
    ...
    public M withDec3Abstract(Consumer<Phase3Abstract<T_ROOT, T_RESULT, CTX>> cons) {
        Objects.requireNonNull(cons);
        M of = me();
    ...
    public TypedQuery<T_RESULT> buildCriteriaQuery(EntityManager em) {
    ...
    public Query buildDeleteOrUpdate(EntityManager em, boolean delete) {
        Query query;
        if (delete) {
            query = em.createQuery(produceDeleteQuery(em));
        } else {
    ...
    public CriteriaDelete<T_ROOT> produceDeleteQuery(EntityManager em) {
    ...
    public CriteriaUpdate<T_ROOT> produceUpdateQuery(EntityManager em) {
        Objects.requireNonNull(em);
        CriteriaBuilder builder = em.getCriteriaBuilder();
    ...
    public CriteriaQuery<T_RESULT> produceCriteriaQuery(EntityManager em) {
        Objects.requireNonNull(em);
        CriteriaBuilder builder = em.getCriteriaBuilder();
