...
public interface SyncDrowConf<R extends SyncDrow, C, N, L, U extends Updates> extends DrowConf<R, C, N, L, U> {
    ...
    public default SyncAndValidationAggregator<R> createAggregator(R row) {
        ...
        Function<ValidArgs<R>, Valid> validationFactoryRow = (arg) -> {
            return createValidation(row, r -> arg.pred.test(row), arg.error);
        };
        return new SyncValidationAggregator<>(row, factory, validationFactory, validationFactoryRow);
    }
